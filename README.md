...
The page is set in Computer Modern — Knuth's TeX faces — loaded from jsDelivr:
`Computer Modern Serif` for prose and `Computer Modern Typewriter` for every figure.
Variables are marked up as `<var>` and render in math italic, the way TeX sets them.

Two things follow from that. Anyone with a TeX distribution installed falls back to
Latin Modern, which is the same design, so the page looks right even if the CDN is
unreachable. And if you would rather not depend on jsDelivr, download the four
`.woff2` files from `vsalvino/computer-modern`, drop them in `fonts/`, and replace the
two `<link>` tags with your own `@font-face` blocks.
