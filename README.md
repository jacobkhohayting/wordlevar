...
- **Lato** (Google Fonts) for everything you read as language: prose, headings,
  buttons, the wordmark. Warm humanist sans, no italics anywhere.
- **Computer Modern Typewriter** (jsDelivr) for everything you read as data: the
  guess log, column headers, section labels, the candidate count, the input field,
  the countdown, the footer.

Symbols and quantities inside running text are set in bold Lato rather than a
separate face, so a sentence stays one texture. Anyone with a TeX distribution
installed gets Latin Modern for the typewriter face, which is the same design.

To change either, edit `--sans` or `--mono` in the `:root` block and swap the
matching `<link>`.
